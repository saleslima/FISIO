...
export function showBookingForm(dateKey, periodIndex, period, day) {
    ...
    periodsList.innerHTML = `
        <div class="booking-form">
            ...
            <label for="bookingCpf">CPF do Paciente:</label>
            <input 
                type="text" 
                id="bookingCpf" 
...
function setupBookingFormHandlers(dateKey, periodIndex, day) {
    ...
    const validateForm = () => {
        ...
        confirmBtn.disabled = !(name.length > 0 && cpf.length === 11 && phone.length === 11);
    };
    ...
    confirmBtn.addEventListener('click', () => {
        ...
        if (name && cpf.length === 11 && phone.length === 11) {
            const bookingData = { name, cpf, phone };
...
export function showConfirmationModal(dateKey, periodIndex, bookingData, day) {
    ...
    const formattedCpf = bookingData.cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4');
...
function generateBookingCard(booking, periodName, periodTime, dateKey, bookingIndex) {
    ...
    const formattedCpf = booking.cpf ? booking.cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4') : 'N/A';
